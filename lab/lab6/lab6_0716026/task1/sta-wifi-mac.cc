...
StaWifiMac::SendDisassociationMsg(void)
{
  ...
  Ptr<Packet> sendPacket = Create<Packet> (buffer, size);
  m_txop->Queue(sendPacket, hdr);
}
      ...
      if (goodBeacon && m_state == ASSOCIATED)
        {
          ...
          SnrTag snrTag;
          copy->RemovePacketTag (snrTag);
          double snr = 10*log10(snrTag.Get());
...
StaWifiMac::UpdateCandidateApList (ApInfo newApInfo)
{
  ...
  double log_snr = 10*log10(newApInfo.m_snr);
  if(log_snr<m_snrThreshold){
    return;
  }
  // Insert before the entry with lower SNR
  for (std::vector<ApInfo>::iterator i = m_candidateAps.begin(); i != m_candidateAps.end(); ++i)
    {
      //std::cout << "ggg" << newApInfo.m_snr << std::endl;
      //std::cout << "iii" << (*i).m_snr <<std::endl;
