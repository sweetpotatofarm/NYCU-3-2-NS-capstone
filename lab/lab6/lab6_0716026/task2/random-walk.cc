...
void Movement2(Ptr<Node> node, Vector m_position);
...
void Movement (Ptr<Node> node, double stepsSize, double stepsTime)
{
  ...
  MOVE step = GetSingleMovement(fp);
  double x = m_position.x + cos(step.rad)*step.speed;
  double y = m_position.y + sin(step.rad)*step.speed;
  if(x>boundary[0].x || x<boundary[1].x || y>boundary[2].y || y<boundary[3].y){
    PrintIllegalMsg(step.index);
  }
  else{
    double old_x = m_position.x;
    double old_y = m_position.y;
    for(int i=0;i<100;i++){
      m_position.x = old_x + (cos(step.rad)*step.speed*(i+1)/100);
      m_position.y = old_y + (sin(step.rad)*step.speed*(i+1)/100);
      Ptr<MobilityModel> mob = node->GetObject<MobilityModel>();
      mob->SetPosition(m_position);
      Simulator::Schedule(Seconds(stepsTime*(i+1)/100), &Movement2, node, m_position);
    }
    //double new_x = m_position.x;
    //double new_y = m_position.y;
    //double new_z = m_position.z;
    //std::cout << "Current position: ( " << new_x << ", " << new_y << ", " << new_z << " )" << std::endl;
    CheckLinkState(node); 
  }
